...
class UrgencyCalculatorTest {
    ...
    void should_return_HIGH_when_parcel_expires_tomorrow() {
        Parcel p = new Parcel(ParcelId.random(), "T1", LocalDate.now(fixedClock).plusDays(1), ParcelStatus.AVAILABLE, pp);
    ...
    void should_return_MEDIUM_when_parcel_expires_in_3_days() {
        Parcel p = new Parcel(ParcelId.random(), "T1", LocalDate.now(fixedClock).plusDays(3), ParcelStatus.AVAILABLE, pp);
        UrgencyCalculator.Result result = calculator.calculate(List.of(p));
    ...
    void should_return_LOW_when_parcel_expires_in_5_days() {
        Parcel p = new Parcel(ParcelId.random(), "T1", LocalDate.now(fixedClock).plusDays(5), ParcelStatus.AVAILABLE, pp);
        UrgencyCalculator.Result result = calculator.calculate(List.of(p));
    ...
    void should_ignore_picked_up_parcels() {
        Parcel p1 = new Parcel(ParcelId.random(), "T1", LocalDate.now(fixedClock).plusDays(1), ParcelStatus.PICKED_UP, pp);
        Parcel p2 = new Parcel(ParcelId.random(), "T2", LocalDate.now(fixedClock).plusDays(5), ParcelStatus.AVAILABLE, pp);
